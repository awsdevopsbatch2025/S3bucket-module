# --- DATA SOURCES ---
# Needed to get the current AWS Account ID for the MRAP routing CLI command
...
# --- NEW MRAP ROUTING RESOURCE ---
# This resource runs the AWS CLI command to set the traffic dial after MRAP is created.
resource "null_resource" "mrap_routing_update" {
  # Run this only if MRAP is configured
  count = var.mrap_name != null && length(var.mrap_regions) > 0 ? 1 : 0

  # Re-run if the MRAP alias or the configuration (names/percentages) changes
  triggers = {
    mrap_alias = aws_s3control_multi_region_access_point.this[0].alias
    config     = jsonencode(var.mrap_regions)
  }

  provisioner "local-exec" {
    # It loops through var.mrap_regions and constructs the traffic updates using the explicit bucket_name.
    command = <<EOT
      aws s3control submit-multi-region-access-point-routes \
      --account-id ${data.aws_caller_identity.current.account_id} \
      --mrap ${aws_s3control_multi_region_access_point.this[0].alias} \
      --route-updates ${join(" ", [
        for r in var.mrap_regions : 
        "Bucket=${r.bucket_name},TrafficDialPercentage=${lookup(r, "traffic_dial_percentage", 100)}"
      ])}
    EOT
  }

  # Ensure this only runs after the MRAP resource is fully created
